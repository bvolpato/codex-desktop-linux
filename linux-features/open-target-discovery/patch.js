...
function applyIdeDiscoveryPatch(currentSource, deps) {
  ...
  const dynamicDiscoveryHelpers = patchedSource.includes("function codexLinuxDiscoveredIdeTargets(")
    ...
    `async function codexLinuxLaunchDesktopEntry(e,t,n,r){let i=codexLinuxFindExecutable(\`gio\`);if(i)try{await codexLinuxLaunchDetached(i,[\`launch\`,e,t]);return}catch{}let a=codexLinuxFindExecutable(\`gtk-launch\`);if(a)try{await codexLinuxLaunchDetached(a,[codexLinuxDesktopEntryLaunchId(e),codexLinuxPathToFileUri(t)]);return}catch{}await codexLinuxLaunchDetached(n,codexLinuxDesktopArgs(r,t))}` +
